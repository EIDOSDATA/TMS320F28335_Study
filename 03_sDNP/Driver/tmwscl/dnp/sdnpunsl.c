  ...
  TMWTYPES_BOOL initialNullUnsol)
{
   ...
   */
  if(pSDNPSession->authSendAggrUnsol && (!initialNullUnsol))
  {  
  ...
  TMWTYPES_BOOL initialNullResponse)
{
...
#endif
  {
   ...
   /* SAv5 says this is not allowed. It was not disallowed in SAv2. */
#if SDNPDATA_SUPPORT_OBJ120 && SDNPCNFG_SUPPORT_SA_VERSION2
    if((((DNPSESN*)pSession)->operateInV2Mode) 
      &&(((SDNPSESN*)pSession)->preChallengeApplConf))
    { 
      sdnpauth_addPreChallengeV1((SDNPSESN*)pSession, pResponse);
    }
#endif
  ...
  TMWSESN *pSession)
{
  ...
  for(index = 0; index < 3; index++)
  {
    ...
    if(pSDNPSession->unsolEventMask & masks[index])
    {
      pSDNPSession->unsolNumPending[index] = (TMWTYPES_USHORT)
        (pSDNPSession->unsolNumPending[index] + sdnprbe_countEvents(pSession, masks[index], TMWDEFS_FALSE, 255));
    }
  ...
  TMWSESN *pSession)
{
...
#endif
  {
    ...
    if(sdnpunsl_countUnsolicitedEvents(pSession) > 0)
    {
      ...
      while(_unsolEventGroups[i].pReadFunc != TMWDEFS_NULL)
      {
        ...
        if(_unsolEventGroups[i].pReadFunc(pSession, TMWDEFS_NULL, pResponse,
          &objHeader, pSDNPSession->unsolEventMask) != SDNPSESN_READ_COMPLETE)
        {
