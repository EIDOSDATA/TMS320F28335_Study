  ...
  TMWTYPES_USHORT *pKeyLength)
{
  ...
  if(_getKey(pAuthInfo->pSession, pSDNPSession->dnp.pCryptoHandle, TMWCRYPTO_USER_UPDATE_KEY, (void*)pUserContext->userNumber, &updateKey)) 
  {
    ...
    { 
      ...
      {
        DNPDIAG_AUTH_ENCRYPT_CYPHER((TMWSESN*)pSDNPSession, pEncryptedValue, encryptedValueLength, TMWDIAG_ID_RX); 
        DNPDIAG_AUTH_ENCRYPT_PLAIN((TMWSESN*)pSDNPSession, pValue, decryptedLength, TMWDIAG_ID_RX); 
      }
#endif
  ...
  TMWTYPES_USHORT  *pMACValueLength)
{
  ...
  if(dnpauth_MACtoLength(algorithm) <= *pMACValueLength)
  {
    ...
    if(pAuthInfo->extraDiags)
    {
      DNPDIAG_AUTH_PLAIN((TMWSESN*)pAuthInfo->pSession, pValue, dataLength, (received) ? TMWDIAG_ID_RX : 0); 
      DNPDIAG_AUTH_HASHED((TMWSESN*)pAuthInfo->pSession, pMACValue, *pMACValueLength, (received) ? TMWDIAG_ID_RX : 0); 
    }
#endif 
  }
  ...
  TMWTYPES_USHORT userNumber)
{
  ...
  if(pUserContext != TMWDEFS_NULL)
  {
    /* Get this in case it has been reconfigured */
  ...
  if(pUserContext != TMWDEFS_NULL)
  {
    ...
    DNPDIAG_SHOW_KEY_STATUS(pSession, userNumber, pAuthInfo->keyChangeSequenceNumber,
      pUserContext->keyWrapAlgorithm, pUserContext->keyStatus, pAuthInfo->MACAlgorithm, 0);
  }
  else
  {
    /* Update the length in the object prefix.  */
    ...
    tmwtarg_store16(&length, pResponse->pMsgBuf + lengthIndex);
    DNPDIAG_SHOW_KEY_STATUS(pSession, userNumber, pAuthInfo->keyChangeSequenceNumber,
      TMWCRYPTO_ALG_KEYWRAP_AES256, DNPAUTH_KEY_AUTHFAIL, pAuthInfo->MACAlgorithm, 0);
  }
...
void TMWDEFS_GLOBAL sdnpsa_sendErrorResponseV7(
  ...
  TMWTYPES_UCHAR errorTextLength) 
{ 
  TMWSESN_TX_DATA *pResponse;
  TMWTYPES_USHORT length;
  TMWTYPES_USHORT lengthIndex; 
  ...
  TMWTYPES_ULONG errorMsgsSentStat = _authSecStatIncrement(pSDNPSession, DNPAUTH_ERROR_MSG_SENT_INDEX);

  /* Get the time now in case we need to log but NOT send the error response */
  sdnputil_getDateTime(pSession, &timeStamp);

  if(errorMsgsSentStat > pAuthInfo->statsMaxValue[SDNPAUTH_ERROR_MSG_SENT_MAX_INDEX])
  {
...
{ 
  ...
  { 
    ...
    if(dnpauth_methodChallengeLength(pAuthInfo->keyChangeMethod) <= length)
    {
      ...
      if(pAuthInfo->extraDiags)
      {
        DNPDIAG_AUTH_ENCRYPT_PLAIN(pSession, pPlainData, plainDataLength, 0); 
        DNPDIAG_AUTH_HASHED(pSession, &pResponse->pMsgBuf[pResponse->msgLength], length, 0); 
      }
#endif 
    }
...
{   
  ...
  if(pUserContext != TMWDEFS_NULL)
  {
    ...
    {
      sdnpdata_authSesnKeyStatCount(pSDNPSession->pDbHandle, pRxFragment->rxAddress, userNumber, 
        pAuthInfo->keyChangeSequenceNumber);  
    }
  }
  ...
  else
  {
    ...
    */
      ...
      &pRxFragment->pMsgBuf[pRxFragment->offset], length))
    {
      ...
      if (userNumber != 0)
      {
        //SPM remove this test code.
        bool ignoreThis = true;
        if (!ignoreThis && statusChangeSequenceNumber <= pAuthInfo->lastSCS)
        {
  ...
  SDNPAUTH_RX_MSG *pRxFragment)
{
  ...
  if(statusChangeSequenceNumber <= pAuthInfo->lastSCS)
  { 
    /* Check for wrap allowing for missed sequence and lastSCS inited to 0xffffff 
    */ 
    if(!((pAuthInfo->lastSCS > 0xfffffff0) && (statusChangeSequenceNumber < 16)))
...
#endif
  {
    ...
    { 
        ...
        pValue2, &valueLength2)))
      {
        ...
        if (pAuthInfo->extraDiags)
        {
          DNPDIAG_AUTH_ENCRYPT_PLAIN((TMWSESN*)pSDNPSession, pValue, valueLength, TMWDIAG_ID_RX);
          DNPDIAG_AUTH_HASHED((TMWSESN*)pSDNPSession, pValue2, valueLength2, TMWDIAG_ID_RX);
        }
#endif 
  ...
  if(errorCode == DNPAUTH_ERROR_NONE)
  {
    ...
    if(userNumber == 0)
    {
      if (operation == DNPAUTH_USER_STATUS_CHANGE)
      {
  ...
  SDNPAUTH_RX_MSG *pRxFragment)
{
  ...
  sdnpsa_sendErrorResponseV7(pSDNPSession, pRxFragment->rxAddress, pAuthInfo->keyChangeSequenceNumber, 0, DNPAUTH_ERROR_UNKNOWN_USER, TMWDEFS_NULL, 0);  
...
static TMWTYPES_BOOL TMWDEFS_LOCAL _processUpdateKeyChangeV13(
  SDNPSESN *pSDNPSession,
  SDNPAUTH_RX_MSG *pRxFragment)
{
  SDNPAUTH_INFO *pAuthInfo; 
  ...
  DNPDIAG_SHOW_AUTH_UPDATEKEYCHANGE((TMWSESN*)pSDNPSession, userNumber, keyChangeSequenceNumber, 
    encryptedUpdateKeyLength, &pRxFragment->pMsgBuf[pRxFragment->offset], TMWDIAG_ID_RX);
  ...
  if(status) 
  {
    ...
    {  
      ...
      status = tmwcrypto_decryptData(pSDNPSession->dnp.pCryptoHandle, dnpauth_methodToKeyTransAlgorithm(pAuthInfo->keyChangeMethod), &key,
  ...
  if(status)
  {
    ...
    if(pAuthInfo->extraDiags)
    {
      DNPDIAG_AUTH_ENCRYPT_CYPHER((TMWSESN*)pSDNPSession, &pRxFragment->pMsgBuf[pRxFragment->offset], encryptedUpdateKeyLength, TMWDIAG_ID_RX);   
      DNPDIAG_AUTH_ENCRYPT_PLAIN((TMWSESN*)pSDNPSession, pValue, decryptedLength, TMWDIAG_ID_RX); 
    }
#endif 
  ...
  const SDNPSESN_AUTH_CONFIG *pConfig)
{
  ...
  /* According to TB2016-002 */
  if (pAuthInfo->randomChallengeDataLength < 4)
    pAuthInfo->randomChallengeDataLength = 4;
  if (pAuthInfo->randomChallengeDataLength > 64)
    pAuthInfo->randomChallengeDataLength = 64;
...
{ 
  ...
  if(DNPDEFS_OBJ_120_AUTHENTICATION == hdr.group)
  {
    ...
    case 3: 
      ...
      /* This removes aggressive mode object from beginning of message */
  ...
  TMWSESN_RX_DATA *pRxFragment)
{
      ...
      if(DNPDEFS_OBJ_120_AUTHENTICATION == hdr.group)
      {
        ...
        case 13:
          ...
          /* Handle this here since it is state machine independent */
          {
            ...
            status = _processUpdateKeyChangeV13((SDNPSESN*)pSession, &rxMsg); 
            ...
            if(status)
            {
              ...
              if(hdr.variation == 14)
              {
                DNPDIAG_SHOW_AUTH_HEADER(pSession, &hdr);
#if DNPCNFG_SUPPORT_AUTHKEYUPDASYM
                status = _processUpdKeyChangeSigV14((SDNPSESN*)pSession, &rxMsg); 
  #endif
              }
              else if(hdr.variation == 15)
              {
                DNPDIAG_SHOW_AUTH_HEADER(pSession, &hdr);
                status = _processUpdateKeyChangeConfV15((SDNPSESN*)pSession, &rxMsg); 
              }
              else
              {
                status = TMWDEFS_FALSE;
  ...
  pUserContext = _findUserContext(pAuthInfo, userNumber);
  if(userNumberRcvd && (pUserContext == TMWDEFS_NULL) && ((event != SDNPAUTH_EVT_ERRORREQ) && (event != SDNPAUTH_EVT_KEYSTATUSREQ)))
  {
  ...
  if(pAuthInfo->state == SDNPAUTH_STATE_WAITFORREPLY)
  {
    ...
    case SDNPAUTH_EVT_AGGRESSIVE_MODE:
      /* According to 1815-2012 Table 7-8, Row 18-22
       */
