  ...
  TMWTYPES_ULONG direction)
{
  ...
  index = DNPLINK_HEADER_LENGTH;
  while(index < numBytes)
  {
    len = tmwtarg_snprintf(buf, sizeof(buf), "%16s", " ");

    rowLength = numBytes - index;
    if(rowLength > DNPLINK_BLOCK_LENGTH)
      rowLength = DNPLINK_BLOCK_LENGTH;
  ...
  TMWTYPES_UCHAR variation)
{
  ...
  case DNPDEFS_OBJ0_CONFIG_SIGNATURE:         return("Configuration signature");
  case DNPDEFS_OBJ0_CONFIG_SIG_ALG:           return("Configuration signature algorithm");
  ...
  TMWDIAG_ID direction)
{
  ...
  tmwtarg_snprintf(buf, sizeof(buf), "%-21s%s Authentication Challenge, User = %d, Sequence = %d, Algorithm = %d-%s, Reason %d-%s\n", 
  ...
  TMWDIAG_ID direction)
{
  ...
  {
    tmwtarg_snprintf(buf, sizeof(buf), "%-21sRx Authentication Reply, User = %d, Sequence = %d %s\n", 
      " ",userNumber, sequenceNumber, (status)? "SUCCESS":"FAILURE");
  }
  else
  {
    tmwtarg_snprintf(buf, sizeof(buf), "%-21sTx Authentication Reply, User = %d, Sequence = %d\n", 
  ...
  TMWDIAG_ID direction)
{
  ...
  { 
    ...
    {
      tmwtarg_snprintf(buf, sizeof(buf), "%-21sRx Authentication Aggressive Mode Request, User = %d, Sequence = %d, Expected Sequence - %d %s\n", 
        " ", userNumber, sequenceNumber, expectedSequence, (status)? "SUCCESS":"FAILURE");
    }
    else
    {
      tmwtarg_snprintf(buf, sizeof(buf), "%-21sRx Authentication Aggressive Mode Request, User = %d, Sequence = %d, %s\n", 
        " ", userNumber, sequenceNumber, (status)? "SUCCESS":"FAILURE");
    }
  }
  else
  { 
    tmwtarg_snprintf(buf, sizeof(buf), "%-21sTx Authentication Aggressive Mode Request, User = %d, Sequence = %d\n", 
      " ", userNumber, sequenceNumber);
  }
  tmwdiag_putLine(&anlzId, buf);
}
...
{ 
  ...
  tmwtarg_snprintf(buf, sizeof(buf), "%-21s%s Authentication Key Status Request, User = %d\n", 
  ...
  TMWDIAG_ID direction)
{
  ...
  tmwtarg_snprintf(buf, sizeof(buf), "%-21s%s Authentication Key Status, User = %d, Sequence = %d, KeyWrapAlgorithm = %s, macAlgorithm = %s, Status %d-%s\n", 
    " ",  (TMWDIAG_ID_RX & direction) ? "Rx":"Tx", userNumber, sequenceNumber, _authKeyWrapAlgorithm(keyWrapAlgorithm), _authMACAlgorithm(macAlgorithm), status, _authKeyStatus(status));
  ...
  TMWTYPES_UCHAR status)
{
  ...
  tmwtarg_snprintf(buf, sizeof(buf), "%-21s%s Authentication Key Status, User = %d, %s  \n", 
...
{ 
  ...
  if(TMWDIAG_ID_RX & direction)
  { 
    tmwtarg_snprintf(buf, sizeof(buf), "%-21sRx Authentication Key Change Request, User = %d, Key Length = %d, Sequence = %d,  Expected = %d %s\n", 
      " ", userNumber, keyLength, sequenceNumber, expectedSequence, (status)? "SUCCESS":"FAILURE");
  }
  else
  { 
    tmwtarg_snprintf(buf, sizeof(buf), "%-21sTx Authentication Key Change Request, User = %d, Key Length = %d, Sequence = %d\n", 
...
{ 
  ...
  tmwtarg_snprintf(buf, sizeof(buf), "%-21sRx Authentication Key Change Request, User = %d, Sequence = %d\n", 
    " ", userNumber, sequenceNumber);
  ...
  TMWDIAG_ID direction)
{
  ...
  if(textLen > 0)
  {
    ...
    tmwtarg_snprintf(buf, sizeof(buf), "%-21s%s Authentication Error, User = %d, linkId = %d, Sequence = %d, Error Code %d-%s, %s\n", 
      timeBuf,  (TMWDIAG_ID_RX & direction) ? "Rx":"Tx", userNumber, linkId, sequenceNumber, errorCode, _authError(errorCode), errorBuf);
  }
  else
  {
    tmwtarg_snprintf(buf, sizeof(buf), "%-21s%s Authentication Error, User = %d, linkId = %d, Sequence = %d, Error Code %d-%s\n", 
      timeBuf,  (TMWDIAG_ID_RX & direction) ? "Rx":"Tx", userNumber, linkId, sequenceNumber, errorCode, _authError(errorCode));
  }
  tmwdiag_putLine(&anlzId, buf);
}
 
void TMWDEFS_GLOBAL dnpdiag_authEncryptPlain(
  TMWSESN *pSession,
  ...
  TMWDIAG_ID direction)
{
  ...
  tmwtarg_snprintf(buf, sizeof(buf), "%-21s Authentication Key Change, plain text\n", " ");
  tmwdiag_putLine(&anlzId, buf);

  /* This displays the 2 byte key length on the first line
   * 16 byte keys on the next two lines 
...
void TMWDEFS_GLOBAL dnpdiag_authEncryptCypher(
  TMWSESN *pSession,
  TMWTYPES_UCHAR *pData,
  TMWTYPES_USHORT length,
  TMWDIAG_ID direction) 
{
  ...
  tmwtarg_snprintf(buf, sizeof(buf), "%-21s Authentication Key Change, encrypted text\n", " ");
  tmwdiag_putLine(&anlzId, buf);
...
void TMWDEFS_GLOBAL dnpdiag_authPlain(
  TMWSESN *pSession,
  TMWTYPES_UCHAR *pData,
  TMWTYPES_USHORT length,
  TMWDIAG_ID direction)
{
  ...
  tmwtarg_snprintf(buf, sizeof(buf), "%-10s      Authentication, plain text\n", " ");
  tmwdiag_putLine(&anlzId, buf);
  ...
  while(index < length)
  {
    ...
    /* Using 17 displays a 1 byte length, 16 byte key on the first two lines */
    /* The data from the Key Status message is on the third line */
    rowLength = length - index;
    if(rowLength > 17)
      rowLength = 17;
...
void TMWDEFS_GLOBAL dnpdiag_authHashed(
  TMWSESN *pSession,
  TMWTYPES_UCHAR *pData,
  TMWTYPES_USHORT length,
  TMWDIAG_ID direction) 
{
  ...
  tmwtarg_snprintf(buf, sizeof(buf), "%-10s      Authentication, hashed text\n", " ");
  tmwdiag_putLine(&anlzId, buf);
  ...
  TMWDIAG_ID direction) 
{
  ...
  tmwtarg_snprintf(buf, sizeof(buf), "%-10s      Authentication, initialization vector\n", " ");
  tmwdiag_putLine(&anlzId, buf);
...
{ 
  ...
  tmwtarg_snprintf(buf, sizeof(buf), "%16s User Name = %s, keyChangeMethod=%d %s, certType=%d\n", 
...
{ 
  ...
  if(direction == TMWDIAG_ID_RX)
    tmwtarg_snprintf(buf, sizeof(buf), "%16s User Name = %s, keyChangeMethod=%d %s, operation=%d %s, role=%d, expires=%d, sequence=%d, last rcvd sequence=%d\n", 
    ...
    tmwtarg_snprintf(buf, sizeof(buf), "%16s User Name = %s, keyChangeMethod=%d %s, operation=%d %s, role=%d, expires=%d, sequence=%d\n", 
...
{ 
  ...
  TMWTARG_UNUSED_PARAM(pChallenge);
  ...
  tmwtarg_snprintf(buf, sizeof(buf), "%16s User Name = %s, keyChangeMethod=%d %s, challenge data length %d \n", 
      " ", nameBuf, keyChangeMethod, _authMethodString(keyChangeMethod), challengeLength); /*SPM show challenge data also. */
  ...
  TMWDIAG_ID direction)
{
  ...
  tmwtarg_snprintf(buf, sizeof(buf), "%16s User %d, Sequence %d, length %d \n", 
      " ", userNumber, sequenceNumber, length); /*SPM show challenge data also. */
  ...
  TMWDIAG_ID direction) 
{
  ...
  tmwtarg_snprintf(buf, sizeof(buf), "%16s User %d, Sequence %d \n", 
      " ", userNumber, sequenceNumber); /*SPM show data also. */
