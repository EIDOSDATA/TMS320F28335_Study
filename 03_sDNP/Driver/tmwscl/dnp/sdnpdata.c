...
{ 
  ...
  return TargDatabase_init(pUserHandle);
#endif
}
...
#else
  /* Put target code here */
  return GetAiDataPoint(pHandle, pointNum);
#endif
}  
  ...
  TMWTYPES_UCHAR *pFlags)
{
  ...
  /*always online*/
  *pFlags = DNPDEFS_DBAS_FLAG_ON_LINE;
  return ReadAiData(pPoint, pValue);
#endif  
  ...
  DNPDEFS_FILE_PERMISSIONS *pPermissions)
{
  ...
  SDNPFSIM_DATABASE *pFDBHandle = (SDNPFSIM_DATABASE *)&(pDb->sdnpFsim);
  return(sdnpfsim_getFileInfo(pSession, pFDBHandle, pFilename,
  ...
  DNPDEFS_FILE_PERMISSIONS *pPermissions)
{
  ...
  return(sdnpfsim_readFileInfo(pSession, pFDBHandle, maxNameSize, pName, pLast, pType, pSize, pTimeOfCreation, pPermissions));
#elif TMWCNFG_USE_MANAGED_SCL
...
TMWTYPES_USHORT TMWDEFS_GLOBAL sdndata_authGetUserRole(
  void           *pHandle,
...
{ 
   ...
   */
  role = sdndata_authGetUserRole(pHandle, userNumber);
