...
void TMWDEFS_GLOBAL tmwtimer_close()
{
  ...
  void *pCallbackParam)
{
  ...
  pTimer = (TMWTIMER *)tmwdlist_getEntry(&_timerPool.list, 0U);
  ...
  while((pTimer != TMWDEFS_NULL) && _checkTimerExpired(pTimer, now))
  {
     ...
     */
    pLockedTimer = (TMWTIMER *)tmwdlist_getEntry(&_timerPool.list, 0U);
  ...
  void *pCallbackParam)
{
  ...
  aTimer = (TMWTIMER *)tmwdlist_getEntry(&_timerPool.list, 0U);
  ...
  TMWCHNL *pChannel)
{
  ...
  if(pTimer != TMWDEFS_NULL)
  {
    ...
    if(pChannel->timerQueue.timerRunning)
    { 
      tmwtarg_cancelMultiTimer(pChannel->timerQueue.handle);
      pChannel->timerQueue.handle = tmwtarg_startMultiTimer(pChannel->pMultiThreadTimerHandle, timeoutValue, _timerCallback, pChannel);  
    }
    else
    {
      /* Start the system timer */
  ...
  void *pCallbackParam)
{
  ...
  /* Sort the timer list based on timeout */
  aTimer = (TMWTIMER *)tmwdlist_getEntry(&pChannel->timerQueue.list, 0U);
  if(aTimer == TMWDEFS_NULL)
  { 
    /* List is empty, add timer */
  ...
  TMWTIMER *pTimer)
{
  if(pTimer->active)
  {
    ...
    {
      tmwtarg_cancelMultiTimer(pTimerQueue->handle);
      pTimerQueue->timerRunning = TMWDEFS_FALSE;
    }
  }
}
