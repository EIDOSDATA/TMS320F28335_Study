  ...
  TMWTYPES_BOOL lockQueue)
{
  ...
  pData = (TMWDB_DATA *)tmwdlist_getEntry(&_tmwdbQueue.data, 0U);
  if(pData != TMWDEFS_NULL)
  {
    if (tmwdlist_removeEntry(&_tmwdbQueue.data, (TMWDLIST_MEMBER *)pData))
    {
#if TMWCNFG_SUPPORT_THREADS
      /* remove the entry and unlock the queue before processing 
       * to prevent the processing from blocking another entry
       * being put on the queue
       */
      if(lockQueue)
      {
        /* Unlock database queue */
