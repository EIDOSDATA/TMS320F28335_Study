...
bool WinTLSChannel::TLSLoadSSLLibrary( void )
{
  if(!OpenSSLLoaded)
  {
    ...
    HINSTANCE hinst = LoadLibrary(_T("ssleay32.dll"));
    if (hinst == NULL) 
    {
      WinIoTarg_formatErrorMsg(m_errorBuffer, WSAGetLastError());   
      LogMessage(TMWDIAG_ID_TARGET | TMWDIAG_ID_ERROR, "TLS, load ssleay32.dll failed\n%19s%s", " ",m_errorBuffer);
      return false;
    }
    hinst = LoadLibrary(_T("libeay32.dll"));
    if (hinst == NULL) 
    {
      WinIoTarg_formatErrorMsg(m_errorBuffer, WSAGetLastError()); 
      LogMessage(TMWDIAG_ID_TARGET | TMWDIAG_ID_ERROR, "TLS, load libeay32.dll failed\n%19s%s", " ",m_errorBuffer);
      return false;
    } 
...
static char *TMW_ciphers = "TLSv1:!SSLv2:!aNULL:!eNULL:!CAMELLIA:!EXPORT40:!EXPORT56:@STRENGTH";
...
bool WinTLSChannel::StartTLSWriterThread(void)
{
  ...
  if (m_TLSWriterThreadIsRunning == false)
  {
    ...
    m_TLSWriterThreadHandle = (HANDLE)WinIoTarg_startThread(TLSWriterThread,this,&threadID,THREAD_PRIORITY_ABOVE_NORMAL);
...
bool WinTLSChannel::TLSListen(SOCKET acceptSocket)
{
  if(m_config.useTLS)
  {
    ...
    SSL_set_bio(m_pSSL, sslbio, sslbio);

    if(!SSL_set_cipher_list(m_pSSL, TMW_ciphers))
    {
    ...
    while(r <=0)
    {
      ...
      if(r<=0)
      {
        retCode = SSL_get_error(m_pSSL, r);
...
bool WinTLSChannel::TLSConnect(SOCKET tempCommSocket)
{
  ...
  while(r <=0)
  {
    ...
    if(m_pSSL == TMWDEFS_NULL)
    {
      LogMessage(TMWDIAG_ID_TARGET | TMWDIAG_ID_ERROR, "TLS, connect error, pSSL == NULL");
      status = false;
      break;
    }

    r = SSL_connect(m_pSSL);
    if(r<=0)
    {
      retCode = SSL_get_error(m_pSSL, r);
      if(retCode != 2)
