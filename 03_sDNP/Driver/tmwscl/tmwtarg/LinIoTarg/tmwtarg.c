...
void * TMWDEFS_GLOBAL tmwtarg_startMultiTimer(
  ...
  void                   *pCallbackParam)
{ 
  /* Instead of calling test code, replace this with your code */
...
void TMWDEFS_GLOBAL tmwtarg_exit()
{
  ...
  _LinuxSemDelete(&LinIoContext.serialSemaphore);
  LinIoContext.serialSemaphore = NULL;
#endif

#if LINIOTARG_SUPPORT_TCP
  int waitCounter = 0;

  /* Shutdown any socket handling threads */
  if (LinIoContext.socketThreadState == LIN_THREAD_RUNNING)
  {
    LinIoContext.socketThreadState = LIN_THREAD_EXITING;
  }
  if (LinIoContext.connectThreadState == LIN_THREAD_RUNNING)
  {
    LinIoContext.connectThreadState = LIN_THREAD_EXITING;
  }

  while ((LinIoContext.socketThreadState > LIN_THREAD_EXITED) ||
    (LinIoContext.connectThreadState > LIN_THREAD_EXITED))
  {
    ...
    if (waitCounter > 100)
    {
      ...
      tmwtarg_snprintf(buf, sizeof(buf), "tmwtarg_exit: socket threads running after count of %d", waitCounter);
      LINIODIAG_ERRORMSG(buf);
      break;
    }
  }

  /* Only delete the semaphore if the threads have exitted. */
  if ((LinIoContext.socketThreadState <= LIN_THREAD_EXITED) &&
    (LinIoContext.connectThreadState <= LIN_THREAD_EXITED))
  {
    _LinuxSemDelete(&LinIoContext.tcpSemaphore);
    LinIoContext.tcpSemaphore = NULL;
    LinIoContext.socketThreadState = LIN_THREAD_IDLE;
    LinIoContext.connectThreadState = LIN_THREAD_IDLE;
    LinIoContext.socketThreadHandle = 0;
    LinIoContext.connectThreadHandle = 0;
  }
#endif
  LinIoContext.bStarted = TMWDEFS_FALSE;
}
...
#if LINIOTARG_SUPPORT_232
  if (pLinIoChannel->type == TMWTARGIO_TYPE_232)
  {
...
#if LINIOTARG_SUPPORT_TCP
  if (pLinIoChannel->type == TMWTARGIO_TYPE_TCP)
  {
