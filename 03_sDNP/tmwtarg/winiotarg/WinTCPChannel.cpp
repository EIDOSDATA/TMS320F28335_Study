...
{  
  ...
  for ( Iter = m_ipAddressList->begin( ) ; Iter != m_ipAddressList->end( ) ; Iter++ )
  {
    ...
    if(strcmp(ptr, "*.*.*.*") != 0) 
    {
      ...
      if (WinTCPChannel::ConvertNameToIpAddress(ptr, AF_INET, &ulongAddress))
      {
        ptr = (char *)inet_ntop(AF_INET, &ulongAddress, tempBuffer, sizeof(tempBuffer));
      }
    }
  ...
  m_commSocket(INVALID_SOCKET), m_clientSocket(INVALID_SOCKET), m_serverSocket(INVALID_SOCKET)
{
  ...
  m_afInet = AF_INET;
#if 0 //Ensure only IPV4 is configured.
...
bool WinTCPChannel::IsLocalAddrConfigured(const char *pAddress)
{
  ...
  if(strlen(m_config.localIpAddress) == 0)
	...
	if(strcmp(m_config.localIpAddress, "0.0.0.0") == 0)
		return true;
...
bool WinTCPChannel::connect(void)
{
  ...
  TMWTYPES_ULONG ulongAddress;
  if (!ConvertNameToIpAddress(ipAddress, AF_INET, &ulongAddress))
  {
  ...
  inet_pton(sockAddr1.sin_family, m_config.localIpAddress, &sockAddr1.sin_addr.s_addr);
    ...
    if(m_config.localUDPPort != WINTCP_UDP_PORT_NONE)
    {
      ...
      if(m_UDPSocket == INVALID_SOCKET)
      {
        ...
        if(m_config.mode == WINTCP_MODE_UDP)
        {
          ...
          ConvertNameToIpAddress(ipAddress, AF_INET, &validUDPAddress);
...
bool WinTCPChannel::udpEndPoint(char *pIpAddressString)
{
  ...
  bool validAddress = ConvertNameToIpAddress(pIpAddressString, m_afInet, &ulongAddress);
  ...
  if(strchr(pIpAddressString, '.') == 0)
  {
    ...
    char addBuf[46];
    LogMessage(TMWDIAG_ID_TARGET , "UDP: Resolved host name to IP Address: %s", inet_ntop(m_afInet, &ulongAddress, addBuf, sizeof(addBuf)));
  }
  ...
  inet_pton(my_addr.sin_family, m_config.localIpAddress, &my_addr.sin_addr.s_addr);
...
#pragma warning(default:4127)
    ...
    else if(FD_ISSET(socket, &readfds))
    {
      ...
      if(returnValue>0)
      {
        ...
        {
          char addBuf[46];
          WinIoInterface::ProtoAnaLog(TMWDIAG_ID_TARGET | TMWDIAG_ID_ERROR, "UDP data discarded from %s", inet_ntop(from.sa_family, &pFrom->sin_addr, addBuf, sizeof(addBuf))); 
        }
      }
    }
	}
  ...
  TMWTYPES_UCHAR UDPPort)
{
  ...
  if(m_srcUDPPort == WINTCP_UDP_PORT_NONE)
  {
    ...
    { 
      m_destSockAddr.sin_port = htons(m_config.destUDPPort);
      inet_pton(m_afInet, m_config.udpBroadcastAddress, &m_destSockAddr.sin_addr.s_addr);
    }
  ...
  { 
    m_destSockAddr.sin_port = htons(m_config.destUDPPort);
    inet_pton(m_afInet, m_config.localIpAddress, &m_destSockAddr.sin_addr.s_addr);
  }
