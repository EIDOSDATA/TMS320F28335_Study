...
#pragma warning(default:4127)
    ...
    if(FD_ISSET(pWinTCPListener->m_socket, &readfds))
    {
      ...
      else
      {
        ...
        char myIPAddress[64];
        inet_ntop(AF_INET, &myAddr.sin_addr, myIPAddress, sizeof(myIPAddress));

        char connectIPAddress[64];
        if (inet_ntop(AF_INET, &remoteAddr.sin_addr, connectIPAddress, sizeof(connectIPAddress)) == WINIOTARG_NULL)
        {
