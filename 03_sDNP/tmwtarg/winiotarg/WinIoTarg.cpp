...
void tmwtargio_initConfig(WINIO_CONFIG *pConfig)
{
  ...
  strcpy(pConfig->targTCP.ipAddress, "127.0.0.1");
  strcpy(pConfig->targTCP.localIpAddress, "0.0.0.0");
  strcpy(pConfig->targTCP.udpBroadcastAddress, "192.168.1.255");
    ...
    if(master || outstation)
    {
      ...
      if(pBinFileTargValues->useSupportedComm1_1_13)
      {
        if(pBinFileTargValues->supportsSerialConn == TMWDEFS_TRUE) 
        {
          ...
          pIoConfig->win232.disabled = TMWDEFS_FALSE;
          strcpy(pIoConfig->win232.baudRate, pBinFileTargValues->baudRate);
          strcpy(pIoConfig->win232.portName, pBinFileTargValues->serialPortName);
        }
...
void *WinIoTarg_Create(const void *pUserConfig,TMWTARG_CONFIG *pTmwTargConfig)
{
  ...
  if (
    pIOConfig->type == WINIO_TYPE_232
    || (pIOConfig->type == WINIO_TYPE_TCP && pIOConfig->targTCP.mode == WINTCP_MODE_CLIENT)
    || (pIOConfig->type == WINIO_TYPE_UDP_TCP && pIOConfig->targTCP.mode == WINTCP_MODE_CLIENT)
  ...
  // This will only set the interfaces that are "allowed" to true.
  if(pIOConfig->useConnectorThread)
...
void WinIoTarg_Destroy(void *pContext)
{
  ...
  // If connector thread is running
  if (pIoConnector->isRunning())
  {
    pWinIoInterface->setTryToConnect(false);
    pIoConnector->DeleteThis = true;
  }
  else 
  {
  ...
  void *pCallbackParam)
{
  ...
  if (pWinIoInterface->isUseConnectorThread() == true)
  {
...
void WinIoTarg_setUseConnectorThread(void *pContext, TMWTYPES_BOOL flag)
{
  WINIO_CONTEXT *pIOContext = (WINIO_CONTEXT *)pContext;
  WinIoInterface *pWinIoInterface = (WinIoInterface*)(pIOContext->pWinIoInterface);
  if (pIOContext->allowConnectorThread == true)
  {
     pWinIoInterface->setUseConnectorThread(flag);
  }
  else
  { // connector thread is not used for servers
     pWinIoInterface->setUseConnectorThread(false);
  }
}
