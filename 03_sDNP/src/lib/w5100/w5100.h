...
#define lanAddr(offset) ((volatile uint16 *)(0x200000 + (uint32)offset))
...
#define PHYSR0                  (COMMON_BASE + 0x003C) //RO
#define PHYSR1                  (COMMON_BASE + 0x003D) //RO
...
extern void setSn_PORTR(SOCKET s, uint8 * addr);
extern void getSn_PORTR(SOCKET s, uint8 * addr);
...
extern void read_data(SOCKET s, uint16 src, volatile uint8 * dst, uint16 len);
extern void write_data(SOCKET s, volatile uint8 * src, uint16 dst, uint16 len);
