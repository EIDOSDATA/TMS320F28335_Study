...
/*
 * Description : writes the data into W5100 registers.
...
{
    Zone7BusTake(W5100S);
#if (__DEF_IINCHIP_BUS__ == __DEF_IINCHIP_DIRECT_MODE__)                /* DIRECT MODE I/F */
    IINCHIP_ISR_DISABLE();
    *((vuint8*)(addr)) = data;
    IINCHIP_ISR_ENABLE();
#elif(__DEF_IINCHIP_BUS__ == __DEF_IINCHIP_INDIRECT_MODE__)             /* INDIRECT MODE I/F */
    IINCHIP_ISR_DISABLE();
     *lanAddr(IDM_ARH) =    (addr >> 8) & 0x00FF;
     *lanAddr(IDM_ARL) =    addr & 0x00FF;
     *lanAddr(IDM_DR)  =    data;
     IINCHIP_ISR_ENABLE();
...
/*
 * Description : reads the value from W5100 registers.
 * Parameter   : Address
 * Return      : Data
 */
uint8 IINCHIP_READ(uint16 addr)
{
    ...
    IINCHIP_ISR_ENABLE();
       ...
       *lanAddr(IDM_ARL) = addr & 0x00FF;
       Delay(0);
       data = *lanAddr(IDM_DR) & 0xFF;
       IINCHIP_ISR_ENABLE();
#elif (__DEF_IINCHIP_BUS__ == __DEF_IINCHIP_SPI_MODE__)             /* SPI MODE I/F */
       IINCHIP_ISR_DISABLE();
    IINCHIP_SpiInit();
    IINCHIP_CSoff();                             // CS=0, SPI start
...
/*
 * Description : writes into W5100 memory(Buffer)
 * Parameter   : Address , buffer address, length
 * Return      : Data
 */
...
{
    IINCHIP_WRITE(SHAR0, addr[0]);
    IINCHIP_WRITE(SHAR1, addr[1]);
    IINCHIP_WRITE(SHAR2, addr[2]);
    IINCHIP_WRITE(SHAR3, addr[3]);
    IINCHIP_WRITE(SHAR4, addr[4]);
    IINCHIP_WRITE(SHAR5, addr[5]);
}
...
void setSn_PORTR(SOCKET s, uint8 * addr)
{
    IINCHIP_WRITE((Sn_PORT0(s) + 0),addr[0]);
    IINCHIP_WRITE((Sn_PORT0(s) + 1),addr[1]);
}

void getSn_PORTR(SOCKET s, uint8 * addr)
{
    addr[0] = IINCHIP_READ(Sn_PORT0(s));
    addr[1] = IINCHIP_READ(Sn_PORT0(s)+1);
}
...
void send_data_processing(SOCKET s, uint8 *data, uint16 len)
{
    ...
    write_data(s, data, (uint16)(ptr), len);
    ptr += len;
...
void recv_data_processing(SOCKET s, uint8 *data, uint16 len)
{
    ...
    read_data(s, (uint16)ptr, data, len); // read data
    ptr += len;
...
*/
void write_data(SOCKET s, volatile uint8 * src, uint16 dst, uint16 len)
{
    uint16 size;
    uint16 dst_mask;
    uint16 dst_ptr;

    dst_mask = (uint16)dst & getIINCHIP_TxMASK(s); //170
    dst_ptr = (uint16)(getIINCHIP_TxBASE(s) + dst_mask); //SBUFBASEADDRESS[s] 16554
    ...
    if (dst_mask + len > getIINCHIP_TxMAX(s))
    {
        ...
        src += size;
        size = len - size;
        dst_ptr = (uint16)(getIINCHIP_TxBASE(s));
        wiz_write_buf((uint16)dst_ptr, (uint8*)src, size);
    }
    else
    {
...
*/
void read_data(SOCKET s, uint16 src, volatile uint8 * dst, uint16 len)
{
    uint16 size;
    uint16 src_mask;
    uint16 src_ptr;
    ...
    src_ptr = (uint16)(getIINCHIP_RxBASE(s) + src_mask);
    ...
    if( (src_mask + len) > getIINCHIP_RxMAX(s) )
    {
        ...
        dst += size;
        size = len - size;
        src_ptr = (uint16)(getIINCHIP_RxBASE(s));
        wiz_read_buf((uint16)src_ptr, (uint8*) dst,size);
    }
    else
    {
