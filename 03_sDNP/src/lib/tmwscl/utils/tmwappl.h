...
extern "C" {
   ...
   * purpose: Initialize SCL
   *  If your environment does not allow initialized data 
   *  this must be called before calling tmwapp_initApplication 
  ...
  * purpose: Initialize the SCL
  *  NOTE This function must be called prior to any calls to tmwappl_initApplication 
  *  in a multi-threaded environment. The call is optional when TMWCNFG_SUPPORT_THREADS
  *  is not defined.
  * arguments:
  *  void
  * returns:
