...
#define TMWPHYSD_CHANNEL_CLOSED(pChannel, reason) \
  TMWTARG_UNUSED_PARAM(pChannel); DEBUG_Printf("%s\n", reason);

#define TMWPHYSD_BYTES_SENT(pChannel, pBuff, numBytes) \
  TMWTARG_UNUSED_PARAM(pChannel); TMWTARG_UNUSED_PARAM(pBuff); DEBUG_Printf("[stack] transmit %d bytes\n", numBytes);

#define TMWPHYSD_BYTES_RECEIVED(pChannel, pBuff, numBytes) \
  TMWTARG_UNUSED_PARAM(pChannel); TMWTARG_UNUSED_PARAM(pBuff); DEBUG_Printf("[stack] received %d bytes\n", numBytes);
