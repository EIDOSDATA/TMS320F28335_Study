...
extern "C" {
   ...
   *  Could also be called from Master or Outstation application if desired to
   ...
   *     Library are the following two USER keys. This function may also be called
   *      by application code to store other types of keys if desired.
   ...
   *    TMWCRYPTO_USER_ASYM_PRV_KEY (application if desired)
   *    TMWCRYPTO_OS_ASYM_PUB_KEY (application if desired)
   *    TMWCRYPTO_OS_ASYM_PRV_KEY (application if desired)
   *    TMWCRYPTO_AUTH_CERT_SYM_KEY (application if desired)
   *    TMWCRYPTO_AUTH_ASYM_PRV_KEY (application if desired)
   *    TMWCRYPTO_AUTH_ASYM_PUB_KEY  (application if desired)
   *      These may be supported for Master and Outstation application
   *      code, but are not required by the Source Code Library.
