...
typedef enum TMWCHNL_ERROR_CODE_ENUM
{
  ...
  /* Link layer errors */
  TMWCHNL_ERROR_LINK_FRAME_LENGTH,        /* Incoming frame exceeded buffer size                 */
...
extern "C" {
  ...
  /* function: tmwchnl_setPolledMode
   * purpose: Tell the channel that is in or out of polled mode
   * arguments:
   *  pChannel - channel to set
   *  polledMode - TMWDEFS_TRUE if polled mode
   * returns:
   *  void
   */
  void tmwchnl_setPolledMode(
    TMWCHNL *pChannel, TMWTYPES_BOOL polledMode);
