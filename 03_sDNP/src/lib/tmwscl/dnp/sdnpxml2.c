...
/* _class0Response */
static TMWTYPES_BOOL _class0Response(SDNPSESN *pSDNPSession, TMWTYPES_UCHAR objectGroup)
{
  int i;
...
static void  _pointListDefinition(SDNPSESN *pSDNPSession, TMWTYPES_UCHAR objectGroup, TMWTYPES_ULONG *curLen, TMWTYPES_BOOL bGetLen)
{
  TMWTYPES_CHAR *pName;
...
static void TMWDEFS_LOCAL _buildDocHeader(SDNPSESN *pSDNPSession, TMWTYPES_ULONG *curLen, TMWTYPES_BOOL bGetLen)
{
  ...
  _appendString(pSDNPSession, "<?xml-stylesheet type=\"text/xsl\" href=\"DNP3DeviceProfileNovember2014.xslt\" media=\"screen\"?>\n", curLen, bGetLen);  
  _appendString(pSDNPSession, "<DNP3DeviceProfileDocument schemaVersion=\"2.10.00\" xmlns=\"http://www.dnp3.org/DNP3/DeviceProfile/November2014\"\n", curLen, bGetLen);
  _appendString(pSDNPSession, "xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\"\n", curLen, bGetLen); 
  _appendString(pSDNPSession, "xsi:schemaLocation=\"http://www.dnp3.org/DNP3/DeviceProfile/November2014 DNP3DeviceProfileNovember2014.xsd\">\n", curLen, bGetLen); 
  pSDNPSession->xml2CurIndent++;
...
static void TMWDEFS_LOCAL _buildBinOutPoint(SDNPSESN *pSDNPSession, void *pDbHandle, TMWTYPES_USHORT pointNum, TMWTYPES_ULONG *curLen, TMWTYPES_BOOL bGetLen)
{
  ...
  if(pPoint != TMWDEFS_NULL)
  {
    ...
    /* We currently don't allow compile out these three function codes */
    _emptyElementString(pSDNPSession, curLen, bGetLen, "supportSelectOperate", TMWDEFS_TRUE);   
    _emptyElementString(pSDNPSession, curLen, bGetLen, "supportDirectOperate", TMWDEFS_TRUE);
    _emptyElementString(pSDNPSession, curLen, bGetLen, "supportDirectOperateNoAck", TMWDEFS_TRUE);
    ...
    _emptyElementString(pSDNPSession, curLen, bGetLen, "countGreaterThanOne", TMWDEFS_TRUE);
    _emptyElementString(pSDNPSession, curLen, bGetLen, "cancelCurrentOperation", TMWDEFS_TRUE);
 
    pSDNPSession->xml2CurIndent--;
    _appendString(pSDNPSession, "</supportedControlOperations>\n", curLen, bGetLen);
    
    /* maxTimeSelectOperate is same for all points */ 
...
#endif
#endif
    /*  NotSupported maxTimeSelectOperate, SCL does not support per point value */ 
...
#endif
    /* New in schema 2.10 */
    /* I think the schmema is incorrect. It should have been
    <xs:simpleType name="groupNumberUsedType">
      <xs:restriction base="xs:string">
        <xs:enumeration value="group110"/>
        <xs:enumeration value="group114"/>
      </xs:restriction>
    </xs:simpleType>
    _valueString(pSDNPSession, "groupNumberUsed", (TMWTYPES_CHAR *)"group110", curLen, bGetLen); 
    */
    _appendString(pSDNPSession, "<groupNumberUsed>\n", curLen, bGetLen);
    _valueString(pSDNPSession, "group110", "group110", curLen, bGetLen);
    _appendString(pSDNPSession, "</groupNumberUsed>\n", curLen, bGetLen);
...
{ 
  ...
  case DNPAUTH_KEYCH_ASYM_RSA1024_SHA1:
    return "<SAES-OAEP-1024WithSHA-1-HMAC/>";
...
#if !SDNPDATA_SUPPORT_OBJ120
  /*notSupported */ 
  _currentValueString(pSDNPSession, "secureAuthenticationSupported",  "<notSupported />", curLen, bGetLen);  
#else

  if(pSDNPSession->authenticationEnabled)
  {
    SDNPAUTH_INFO *pAuthInfo = (SDNPAUTH_INFO*)pSDNPSession->pAuthenticationInfo;
    TMWTYPES_ULONG maxErrorCount;
...
static void _buildAnalogInputGroup(SDNPSESN *pSDNPSession, TMWTYPES_ULONG *curLen, TMWTYPES_BOOL bGetLen)
{
 ...
 /*  
  ...
  if(sdnpdata_XmlGetAnlgInGroupConfig(pSDNPSession->pDbHandle, &config))
  {
    ...
    /* SCL does not support frozen analog inputs */
    _appendString(pSDNPSession, "<defaultFrozenAnalogInputStaticVariation />\n", curLen, bGetLen);
    _appendString(pSDNPSession, "<defaultFrozenAnalogInputEventVariation />\n", curLen, bGetLen);
    _appendString(pSDNPSession, "<frozenAnalogInputClass0ResponseMode />\n", curLen, bGetLen);
    _appendString(pSDNPSession, "<frozenAnalogEventReportingMode />\n", curLen, bGetLen);
  }
  ...
  /* New in schema 2.09 */
  _appendString(pSDNPSession, "<frozenEventBufferPerObjectGroup />\n", curLen, bGetLen);
...
{ 
  ...
  */
  ...
  /* fileTransferSupported */
#if !SDNPDATA_SUPPORT_OBJ70
  _currentValueString(pSDNPSession, "fileTransferSupported", _yesNoString(TMWDEFS_FALSE), curLen, bGetLen);
#else
  _currentValueString(pSDNPSession, "fileTransferSupported", _yesNoString(TMWDEFS_TRUE), curLen, bGetLen);  
