...
#if SDNPDATA_SUPPORT_OBJ20 && SDNPDATA_SUPPORT_OBJ21
...
#if SDNPDATA_SUPPORT_OBJ120
  if(pSDNPSession->authSendAggrResp)
  {  
  ...
  TMWTYPES_BOOL firstFragment)
{
...
#if SDNPDATA_SUPPORT_OBJ120
  if(pSDNPSession->preChallengeApplConf)
  ...
  if(pResponse->txFlags & TMWSESN_TXFLAGS_CONTAINS_EVENTS)
  {
    ...
    {
      int i;
      sdnpunsl_countUnsolicitedEvents(pSession);
      for(i = 0; i < 3; i++)
      {
  ...
  TMWSESN *pSession)
{
  ...
  if(pSDNPSession->pCurrentMessage != TMWDEFS_NULL)
    return;
  ...
  if(pSDNPSession->pendingRequest)
  {
    ...
    {
      DNPDIAG_ERROR(pSession->pChannel, TMWDEFS_NULL, DNPDIAG_FC_DISABLED);
    }
    else
    { 
      ...
      while(sdnpsesn_funcTable[index].pFunc != TMWDEFS_NULL)
      {
        if(pSDNPSession->lastRcvdRequest.fc == sdnpsesn_funcTable[index].funcCode)
        {
          ...
          {
            DNPDIAG_ERROR(pSession->pChannel, TMWDEFS_NULL, DNPDIAG_BRDCST_DISABLED);
            break;
          }
  ...
  if(pSDNPSession->unsolAllowed)
  {
      ...
      && !pSDNPSession->unsolQueued)
    {
      ...
      {
        /* Yep, send events */
        sdnpunsl_processUnsolEvents(pSession);
      }
    }
  }
}
  ...
  TMWSESN_RX_DATA *pRxFragment)
{
  ...
  if((pSDNPSession->pCurrentMessage != TMWDEFS_NULL)
    && ((pSDNPSession->pCurrentMessage->pMsgBuf[DNPDEFS_AH_INDEX_APPL_CTRL] & DNPDEFS_AC_UNSOLICITED) == 0))
  {
    SDNPDIAG_ERROR(pSession->pChannel, pSession, SDNPDIAG_CANCEL_FRAGMENT);

    /* dnpchnl_cancelFragment will NULL out pCurrentMessage */
    dnpchnl_cancelFragment(pSDNPSession->pCurrentMessage);
  }
  ...
  DNPCHNL_RESP_STATUS status)
{
  ...
  if(pSDNPSession->pCurrentMessage == pTxData)
  {
    pSDNPSession->pCurrentMessage = TMWDEFS_NULL;
  }
  ...
  /* See if this is a timeout of an unsolicited response  
   * This may have timed out before it could be transmitted and pTxData deleted 
   */
  if((pTxData == TMWDEFS_NULL)
    ||((pTxData->pMsgBuf[0] & DNPDEFS_AC_UNSOLICITED) != 0)) 
  {
    int index;
  ...
  TMWSESN_TX_DATA *pTxData)
{
  ...
  { 
    ...
    pSDNPSession->pCurrentMessage = pTxData;
    pTxData->pChannel->pCurrentMessage = pTxData;
  }
  ...
  TMWSESN_TX_DATA *pTxData)
{
  ...
  {
    if(pTxData == pSDNPSession->pCurrentMessage)
      pSDNPSession->pCurrentMessage = TMWDEFS_NULL;
  }
  ...
  TMWSESN_TX_DATA *pTxData)
{
  ...
  ((SDNPSESN *)pSession)->pCurrentMessage = TMWDEFS_NULL;
  ...
  else
  {
    ...
    if(pSDNPSession->pCurrentMessage != TMWDEFS_NULL)
    {
      if(pSDNPSession->lastRespSequenceNumber == sequenceNumber)
      {
         ...
         */
        pChannel->pTprt->pTprtCancel(pChannel->pTprtContext, pSDNPSession->pCurrentMessage);

        /* Application Response */
        tmwdlist_removeEntry(&pChannel->messageQueue, (TMWDLIST_MEMBER *)pSDNPSession->pCurrentMessage);

        dnpchnl_freeTxData(pSDNPSession->pCurrentMessage);
  ...
  TMWSESN *pSession)
{
  ...
  while(pRxMessage->offset < pRxMessage->msgLength)
  {
    ...
    while(_sdnpObjFreezeFuncTable[index].pFreezeFunc != TMWDEFS_NULL)
    {
      ...
      {
        if(!_sdnpObjFreezeFuncTable[index].pFreezeFunc(pSession, pRxMessage->fc, &header))
        {
   ...
   * 밊unction Code Not Implemented� indication. Devices that support Frozen Counters
  ...
  SDNPSESN_CONFIG *pConfig)
{
  ...
  pConfig->source = 4;
  pConfig->destination = 1024;

  pConfig->active = TMWDEFS_TRUE;

  pConfig->linkStatusPeriod             = TMWDEFS_SECONDS(10);  // default 0
  pConfig->linkStatusTimeoutDisconnect  = TMWDEFS_FALSE;//TMWDEFS_TRUE;
  ...
  pConfig->applConfirmTimeout           = TMWDEFS_SECONDS(0);//TMWDEFS_SECONDS(10);
  pConfig->unsolConfirmTimeout          = TMWDEFS_SECONDS(10);
  pConfig->selectTimeout                = TMWDEFS_SECONDS(5);

  pConfig->respondNeedTime              = TMWDEFS_FALSE;//TMWDEFS_TRUE;
  ...
  pConfig->unsolAllowed                 = TMWDEFS_FALSE;
  pConfig->sendUnsolWhenOnline          = TMWDEFS_FALSE;
  pConfig->unsolSendIdenticalRetry      = TMWDEFS_TRUE;//TMWDEFS_FALSE;
  pConfig->unsolClassMask               = TMWDEFS_CLASS_MASK_NONE;
  pConfig->unsolMaxRetries              = 3;
  pConfig->unsolRetryDelay              = TMWDEFS_SECONDS(0);//TMWDEFS_SECONDS(5);
  pConfig->unsolOfflineRetryDelay       = TMWDEFS_SECONDS(0);//TMWDEFS_SECONDS(30);
  ...
  pConfig->obj30DefaultVariation  = 5;  /* 32 Bit Analog Input w/o flag */
  pConfig->obj32DefaultVariation  = 1;  /* 32 Bit Analog Change Event w/o time */
  pConfig->obj34DefaultVariation  = 2;  /* 32 Bit Analog Deadband */
  pConfig->obj40DefaultVariation  = 3;  /* 16 Bit Analog Output Status, (32 Bit var 1 not in subset level 2 */
  ...
  pConfig->binaryInputMaxEvents = 0;
  ...
  pConfig->doubleInputMaxEvents = 0;
  ...
  pConfig->binaryOutputMaxEvents = 0;
  ...
  pConfig->binaryOutCmdMaxEvents = 0;
  ...
  pConfig->analogInputMaxEvents = 0;
  ...
  pConfig->binaryCounterMaxEvents = 0;
  ...
  pConfig->frozenCounterMaxEvents = 0;
  ...
  pConfig->analogOutputMaxEvents = 0;
  ...
  void *pUserHandle)
{
  ...
  /* Current response message outstanding on this channel */
  pSDNPSession->pCurrentMessage = TMWDEFS_NULL;
  ...
  /* Initialize IIN */
  pSDNPSession->iin = 0;//DNPDEFS_IIN_RESTART;
