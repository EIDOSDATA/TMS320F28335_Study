  ...
  TMWTYPES_USHORT encryptedValueLength)
{
  ...
  { 
    ...
    if(pInfo->extraDiags)
    {
      ...
      DNPDIAG_AUTH_ENCRYPT_PLAIN((TMWSESN*)pSDNPSession, valuePtr, decryptedLength, TMWDIAG_ID_RX); 
    }
#endif
...
#if TMWCNFG_SUPPORT_DIAG
  if(pInfo->extraDiags)
    DNPDIAG_AUTH_PLAIN((TMWSESN*)pInfo->pSession, valuePtr, dataLength, 0); 
#endif
...
#if TMWCNFG_SUPPORT_DIAG
    if(pInfo->extraDiags)
      DNPDIAG_AUTH_HASHED((TMWSESN*)pInfo->pSession, pMACValue, *pMACValueLength, 0); 
#endif
    return TMWDEFS_TRUE;
  } 
  ...
  TMWTYPES_USHORT bytesInObject)
{
  ...
  if(pUserContext != TMWDEFS_NULL)
  {
    ...
    if(bytesInObject == _macLength(pInfo->MACAlgorithm))
    {
      ...
      {
        if(_determineAggrMACValue(pInfo, 
          pInfo->MACAlgorithm, &pUserContext->controlSessionKeyV2,
          pInfo->lastChallengeSent, pInfo->lastChallengeSentLength,
...
{ 
  ...
  status = _determineMACValue(pInfo, 
  ...
  TMWTYPES_USHORT userNumber)
{
  ...
  { 
    ...
    if(!_determineMACValue(pInfo, pInfo->MACAlgorithm, &pUserContext->monitorSessionKeyV2, pUserContext->lastKeyChange, 
