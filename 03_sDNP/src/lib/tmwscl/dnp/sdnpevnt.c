  ...
  SDNPDATA_ADD_EVENT_VALUE *pValue)
{
  ...
  switch(group)
  {
    ...
    case DNPDEFS_OBJ_111_STRING_EVENTS:
    {
      ...
      ptr->strLength = pValue->stringPtr.length;
      break;
    }
#endif
...
#if SDNPDATA_SUPPORT_OBJ120
    if(((SDNPSESN *)pResponse->pSession)->preChallengeApplConf)
 ...
 SDNPEVNT_DATA *pEvent)
{
  ...
  { 
    ...
    case 113:
      ...
      break;
    case 122:
      pEvent->obj122.assocId = 0;
      pEvent->obj122.value = userEvent.value.ulValue;
      break;
    } 
    return(TMWDEFS_TRUE);
  }
  ...
  if((sdnpevnt_countEvents(pSession, classMask, pDesc, TMWDEFS_FALSE, 1)) > 0)
  {
    ...
    while(pEvent != TMWDEFS_NULL)
    {
      ...
      if((pEvent->classMask & classMask) != 0)
      {
        ...
        { 
          /* For data set events, we need to be able to this event won't fit */
  ...
  SDNPEVNT_DESC *pDesc)
{
  ...
  if((sdnpevnt_countEvents(pSession, classMask, pDesc, TMWDEFS_FALSE, 1)) > 0)
  {
    ...
    maxMsgLength = pResponse->maxLength;
#if SDNPDATA_SUPPORT_OBJ120
    if(((SDNPSESN *)pSession)->preChallengeApplConf)
      /* leave room for challenge object g120v1 at end of fragment */
      maxMsgLength -=32;
#endif
