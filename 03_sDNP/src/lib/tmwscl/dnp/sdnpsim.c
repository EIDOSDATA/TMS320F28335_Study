  ...
  TMWTYPES_BOOL clearAfterFreeze)
{
  ...
  sdnputil_getDateTime((TMWSESN*)pFrozenCounterPoint->pSCLHandle, &pFrozenCounterPoint->timeStamp);
...
static void _createDeviceAttributes(SDNPSIM_DATABASE *pDbHandle)
{
  ...
  data.value.pStrValue = (TMWTYPES_UCHAR *)"1234";
  data.length = (TMWTYPES_UCHAR)strlen((char *)data.value.pStrValue);
  pPoint = (TMWSIM_POINT *)sdnpsim_addDeviceAttribute(pDbHandle, 0, DNPDEFS_OBJ0_CONFIG_SIGNATURE, 0, &data); 
  tmwsim_setDescription(pPoint, "Configuration signature"); 

  data.type = DNPDEFS_ATTRIBUTE_TYPE_VSTR;
  data.value.pStrValue = (TMWTYPES_UCHAR *)"CCITT CRC16";
  data.length = (TMWTYPES_UCHAR)strlen((char *)data.value.pStrValue);
  pPoint = (TMWSIM_POINT *)sdnpsim_addDeviceAttribute(pDbHandle, 0, DNPDEFS_OBJ0_CONFIG_SIG_ALG, 0, &data); 
  tmwsim_setDescription(pPoint, "Configuration signature algorithm"); 
...
{ 
  ...
  pConfig->permitUserCredentialExpiry = TMWDEFS_FALSE;
