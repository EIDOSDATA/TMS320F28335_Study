 ...
 * TB2016-002 suggests limiting challenge data size portion of status message to 64.
 * If that rule is followed, this could be set to 80
 */
...
typedef struct SDNPAuthenticationInfo {
   ...
   * before notifying human and sending a g120v7 error message on other assocations. 
   */
  ...
  /* extra diagnostics including encryption details and state machine event logging */
  ...
  /* List of user contextes, one context per user number */
