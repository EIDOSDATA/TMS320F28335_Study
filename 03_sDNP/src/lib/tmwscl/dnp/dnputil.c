...
TMWTYPES_USHORT dnputil_linkFrameSizeToTprt(
  ...
  DNPDATA_DATASET_VALUE   *pValue)
{
  ...
  case DNPDATA_VALUE_SFLT:
    tmwtarg_store32((TMWTYPES_ULONG *)&pValue->value.sfltValue, &pMsgData[msgLength]);
    break;
  case DNPDATA_VALUE_DOUBLE:
    {
      ...
      if(length == 4)
      {
        ...
        tmwtarg_store32((TMWTYPES_ULONG *)&temp, &pMsgData[msgLength]);
      }
      else
      {
        tmwtarg_store64((TMWTYPES_DOUBLE *)&pValue->value.doubleValue, &pMsgData[msgLength]);
      }
    }
    break; 
  ...
  DNPDATA_ATTRIBUTE_VALUE *pValue)
{
  ...
  case DNPDEFS_ATTRIBUTE_TYPE_FLT:
    if(pValue->length == 4)
      tmwtarg_store32((TMWTYPES_ULONG *)&pValue->value.fltValue.sfltValue, &pTxData->pMsgBuf[pTxData->msgLength]);
    else
      tmwtarg_store64((TMWTYPES_DOUBLE *)&pValue->value.fltValue.doubleValue, &pTxData->pMsgBuf[pTxData->msgLength]);
    break; 
