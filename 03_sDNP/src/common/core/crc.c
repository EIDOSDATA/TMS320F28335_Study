...
uint16 Compute_DNP_CRC(uint16 *pBuf, uint16 length)
{
   ...
   while(length--)
   {
       uint8 High = (uint8)(*pBuf >> 8);
       uint8 Low  = (uint8)((*pBuf) & 0x00FF);
       CRC = (uint16)((CRC >> 8) ^ (CrcTable[((CRC ^ Low)& 0x00ff)]));
       CRC = (uint16)((CRC >> 8) ^ (CrcTable[((CRC ^ High)& 0x00ff)]));
       pBuf++;
   }
   return ~CRC;
}
